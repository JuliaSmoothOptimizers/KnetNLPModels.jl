...
function set_vars!(model::ChainedNLPModel, new_w :: Vector) 
...
"""
build_nested_array_from_vec(model::ChainedNLPModel, v::Vector{T}) where T <: Number = begin build_nested_array_from_vec!(model, v); model.nested_knet_array end 
function build_nested_array_from_vec!(model::ChainedNLPModel, v::Vector{T}) where T <: Number
...
function NLPModels.obj(nlp :: ChainedNLPModel, w :: AbstractVector{Y}) where Y <: Number
...
NLPModels.grad(nlp :: ChainedNLPModel, w :: AbstractVector{Y}) where Y <: Number = begin g = similar(w); grad!(nlp, w, g); return g end
function NLPModels.grad!(nlp :: ChainedNLPModel, w :: AbstractVector{Y}, g :: AbstractVector{Y}) where Y <: Number
...
"""
reset_minibatch_train!(nlp :: ChainedNLPModel) = nlp.minbatch_train = create_minibatch(nlp.data_train[1], nlp.data_train[2], nlp.size_minibatch)

"""
		reset_minibatch_test!(nlp)	
Take a new minibatch for the ChainedNLPModel. Usually use before a new accuracy test.
"""
reset_minibatch_test!(nlp :: ChainedNLPModel) = nlp.minbatch_test = create_minibatch(nlp.data_test[1], nlp.data_test[2], nlp.size_minibatch)

""" 
		accuracy(nlp)
Computes the accuracy of the network nlp.chain given the data in nlp.minbatch_test.
"""
accuracy(nlp :: ChainedNLPModel{T,S,C}) where {T,S,C} = Knet.accuracy(nlp.chain; data=nlp.minbatch_test)